...
export class HeroesPage {
  ...
  addHero() {
    this.loadingService.run(() => this.router.navigate(['/heroes', 'new']), 400);
  }

  editHero(hero: Hero) {
    this.loadingService.run(() => this.router.navigate(['/heroes', hero.id, 'edit']), 400);
  }

  viewHero(hero: Hero) {
    this.loadingService.run(() => this.router.navigate(['/heroes', hero.id]), 400);
  }
  ...
  deleteHero(hero: Hero): void {
    ...
    dialogRef.afterClosed().subscribe(async (confirmed) => {
      ...
      await this.loadingService.run(() => this.heroService.deleteHero(hero.id), 400);
    });
  }
}
